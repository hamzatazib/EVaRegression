...
# Read data
train = CSV.read("/pool001/htazi/MLProject/concrete_train_1.csv")
test = CSV.read("/pool001/htazi/MLProject/concrete_test_1.csv")

#From DataFrames to Matrices
#input data
X = convert(Matrix, train[:,1:8])
X_test = convert(Matrix, test[:,1:8])
...
function gradient_sobjective_l2(X,Y,s,w,alpha,lambda) # Not affected bY lambda but still putting it for function homogeneity issue
    n = size(X)[1]
...
function gradient_wobjective_l2(X,Y,s,w,alpha,lambda)
    n = size(X)[1]
    return 2*lambda*w + 2*sum(((dot(w,X[i,:])-Y[i]))*exp(1/s*(dot(w,X[i,:])-Y[i])^2)*X[i,:] for i=1:n)/sum(exp(1/s*(dot(w,X[i,:])-Y[i])^2) for i=1:n)
end

function gradient_descent_EVaR_l2(X, Y, s_0, w_0, alpha, lambda, c_s, c_w, epsilon)
    """
    c_s: constant learning rate associated to the gradient_ w.r.t s
    c_w: constant learning rate associated to the gradient_ w.r.t w
    epsilon: 
    """
    #println(objective_l2(X,Y,s_0,w_0,alpha,lambda))
    list_f = [objective_l2(X,Y,s_0,w_0,alpha,lambda)]
    s=s_0; w=w_0
    n_grad = dot(gradient_wobjective_l2(X,Y,s,w,alpha,lambda), gradient_wobjective_l2(X,Y,s,w,alpha,lambda)) + dot(gradient_sobjective_l2(X,Y,s,w,alpha,lambda),gradient_sobjective_l2(X,Y,s,w,alpha,lambda))
    k=1
    while n_grad > epsilon
        if k == 30000
            break
        end
        s = s - c_s*gradient_sobjective_l2(X,Y,s,w,alpha,lambda)
        s = max(0.000000001,s)
        w = w - c_w*gradient_wobjective_l2(X,Y,s,w,alpha,lambda)
        n_grad = dot(gradient_wobjective_l2(X,Y,s,w,alpha,lambda), gradient_wobjective_l2(X,Y,s,w,alpha,lambda)) + dot(gradient_sobjective_l2(X,Y,s,w,alpha,lambda),gradient_sobjective_l2(X,Y,s,w,alpha,lambda))
...
function nesterov_gradient_descent_EVaR_l2(X, Y, s_0, w_0, alpha, lambda, c_s, c_w, epsilon)
       """
    c_s: constant learning rate associated to the gradient_ w.r.t s
    c_w: constant learning rate associated to the gradient_ w.r.t w
    epsilon: stopping gradient norm
    μ: momentum
    """
    #println(objective_l2(X,Y,s_0,w_0,alpha,lambda))
    list_f = [objective_l2(X,Y,s_0,w_0,alpha,lambda)]
    ...
    while n_grad > epsilon
        ...
        s = (s - c_s*gradient_sobjective_l2(X,Y,s,w,alpha,lambda))*(1+μ) - μ*s
        s = max(0.000000001,s)
        w = (w - c_w*gradient_wobjective_l2(X,Y,s,w,alpha,lambda))*(1+μ) - μ*w
        n_grad = dot(gradient_wobjective_l2(X,Y,s,w,alpha,lambda), gradient_wobjective_l2(X,Y,s,w,alpha,lambda)) + dot(gradient_sobjective_l2(X,Y,s,w,alpha,lambda),gradient_sobjective_l2(X,Y,s,w,alpha,lambda))
        push!(list_f, objective_l2(X,Y,s,w,alpha,lambda))
        k = k+1
    end
...
function gradient_sobjective_l1(X,Y,s,w,alpha,lambda) # Not affected bY lambda but still putting it for function homogeneity issue
    n = size(X)[1]
...
function gradient_wobjective_l1(X,Y,s,w,alpha,lambda)
    n = size(X)[1]
    return 2*lambda*w + sum(sign.((dot(w,X[i,:])-Y[i]))*exp(1/s*(abs(dot(w,X[i,:])-Y[i])))*X[i,:] for i=1:n)/sum(exp(1/s*(abs(dot(w,X[i,:])-Y[i]))) for i=1:n)
end

function gradient_descent_EVaR_l1(X, Y, s_0, w_0, alpha, lambda, c_s, c_w, epsilon)
    """
    c_s: constant learning rate associated to the gradient_ w.r.t s
    c_w: constant learning rate associated to the gradient_ w.r.t w
    epsilon: 
    ...
    s=s_0; w=w_0
    n_grad = dot(gradient_wobjective_l1(X,Y,s,w,alpha,lambda), gradient_wobjective_l1(X,Y,s,w,alpha,lambda)) + dot(gradient_sobjective_l1(X,Y,s,w,alpha,lambda),gradient_sobjective_l1(X,Y,s,w,alpha,lambda))
    k=1
    while n_grad > epsilon
        s = s - c_s*gradient_sobjective_l1(X,Y,s,w,alpha,lambda)
        s = max(0.000000001,s)
        w = w - c_w*gradient_wobjective_l1(X,Y,s,w,alpha,lambda)
        n_grad = dot(gradient_wobjective_l1(X,Y,s,w,alpha,lambda), gradient_wobjective_l1(X,Y,s,w,alpha,lambda)) + dot(gradient_sobjective_l1(X,Y,s,w,alpha,lambda),gradient_sobjective_l1(X,Y,s,w,alpha,lambda))
...
function nesterov_gradient_descent_EVaR_l1(X, Y, s_0, w_0, alpha, lambda, c_s, c_w, epsilon)
        """
    c_s: constant learning rate associated to the gradient_ w.r.t s
    c_w: constant learning rate associated to the gradient_ w.r.t w
    epsilon:
    ...
    while n_grad > epsilon
        μ = k/(k+3)
        s = (s - c_s*gradient_sobjective_l1(X,Y,s,w,alpha,lambda))*(1+μ) - μ*s
        s = max(0.000000001,s)
        w = (1+μ).*(w - c_w*gradient_wobjective_l1(X,Y,s,w,alpha,lambda)) - μ.*w
        n_grad = dot(gradient_wobjective_l1(X,Y,s,w,alpha,lambda), gradient_wobjective_l1(X,Y,s,w,alpha,lambda)) + dot(gradient_sobjective_l1(X,Y,s,w,alpha,lambda),gradient_sobjective_l1(X,Y,s,w,alpha,lambda))
        push!(list_f, objective_l1(X,Y,s,w,alpha,lambda))
        if k == 30000
            break
...
for train_prop in [0.7,0.6,0.5]
    ...
    for lambda_EVaR in [0, 0.01, 0.1, 0.2, 1, 10, 20, 50, 100]
        for c_s_EVaR in [0.0001, 0.001, 0.01, 0.1]
            for c_w_EVaR in [0.0001, 0.001, 0.01, 0.1]
                list_f_opt, s_opt, w_opt_EVaR = nesterov_gradient_descent_EVaR_l2(X,Y,s_0,w_0,alpha,lambda_EVaR,c_s_EVaR,c_w_EVaR,epsilon)
    ...
    list_f_opt, s_opt, w_opt_EVaR = nesterov_gradient_descent_EVaR_l2(
    ...
    for lambda_EVaR_l1 in [0, 0.01, 0.1, 0.2, 1, 10, 20, 50, 100]
        for c_s_EVaR_l1 in [0.0001, 0.001, 0.01, 0.1]
            for c_w_EVaR_l1 in [0.0001, 0.001, 0.01, 0.1]
                list_f_opt, s_opt, w_opt_EVaR_l1 = nesterov_gradient_descent_EVaR_l1(X,Y,s_0,w_0,alpha,lambda_EVaR_l1,c_s_EVaR_l1,c_w_EVaR_l1,epsilon)
    ...
    # Fitting Stable regression with best lambda
    list_f_opt, s_opt, w_opt_EVaR_l1 = nesterov_gradient_descent_EVaR_l1(
